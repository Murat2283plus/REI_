...
class REI(object):
    ...
    def train_rei(self, dataloader, physics, transform, epochs, lr, alpha, ckp_interval,
        ...
        # criterion = torch.nn.MSELoss().to(self.device) #murat 误差
        criterion = torch.nn.SmoothL1Loss().to(self.device) #murat 误差
        ...
        for epoch in range(epochs):
            ...
            if save_ckp:
                if epoch >0 and epoch % ckp_interval == 0:
                    ...
                    torch.save(generator.state_dict(), './ct.pt')#murat保存模型
            if epoch + 1 == epochs:
                state = {'epoch': epoch,
                         'state_dict': generator.state_dict(),
                         'optimizer': optimizer.state_dict(),
