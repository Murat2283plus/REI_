...
class AbstractFilter(nn.Module):
    ...
    def forward(self, x):
        ...
        projection = torch.fft.rfft(padded_tensor.transpose(2,3), 1).transpose(2,3) * fourier_filter
        return torch.fft.irfft(projection.transpose(2,3), 1).transpose(2,3)[:,:,:input_size,:]
    ...
    def _get_fourier_filter(self, size):
        ...
        fourier_filter = torch.fft.rfft(f, 1)
        # print(fourier_filter.shape,'88murat')
        # print(fourier_filter)
        # print(size)
