import torch #Murat
import numpy as np #Murat
from models.unet import UNet #Murat


# from dataset.mridb import MRIData
# from dataset.cvdb import CVDB_CVPR
from dataset.ctdb import CTData #Murat

# from physics.mri import MRI
from physics.inpainting import Inpainting
from physics.ct import CT #Murat

from utils.metric import cal_psnr, cal_psnr_complex  #Murat
...
def test_ct(net_name, net_ckp, device):  #Murat
    radon_view = 50
    I0 = 1e5
    sigma = 30
    ...
    dataloader = torch.utils.data.DataLoader( #murat作者写的train 不知道为啥，这里改成test
        dataset=CTData(mode='test'), batch_size=2, shuffle=True)
    ...
    for i, x in enumerate(dataloader):
        ...
        checkpoint = torch.load(net_ckp, map_location=device)
        # unet.load_state_dict(checkpoint['state_dict'])
        unet.load_state_dict(checkpoint)#murat
        unet.to(device).eval()
        x_net = f(fbp)
